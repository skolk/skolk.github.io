...
- **Network memory**: every network the daemon sees gets its mode settings remembered, no pinning required. The lowdata family plus Solo (on or off, and which app) are written to `~/.netmeter/networks.json` as they change, keyed by gateway MAC, and rejoining a remembered network brings back how it was last used: Low Data comes on at the coffee shop because it was on there last week, Solo stays off at home because you never solo at home. Explicit profiles still win where both exist, but only for the keys they pin; memory answers for the rest, which is what keeps a solo mode from the previous network from sailing through a profiled join. `netmeter networks` lists what is remembered, `networks rm NAME|MAC` forgets one, `networks off` stops the remembering (entries stay, nothing applies or updates).
- **Menu**: per-app list switchable between Session and Today, each row with an on/off switch that freezes the app (SIGSTOP) until resumed. The list shows the top 8; a "more" row under them expands the next dozen in place and collapses them again, the state persisting across opens. A frozen app past the fold stays visible even when collapsed, because the switch that unfreezes it must not hide behind the row that lists it. The switch shows **both** totals at once with how long each has been accumulating (`Session ⇅351MB · 3m` / `Today ⇅2.01GB · 8h 59m`), so the other number never costs a click; the selected side is the one in full contrast. **Reset Session** sits under that control, beside the counter it resets, with the session's start time next to it. The running duration is not repeated there, because the control itself already carries it.
...
- **CLI**: `netmeter`, `netmeter session`, `netmeter session reset`, `netmeter pause <app>`, `netmeter throttle <app> --pct 25`, `netmeter throttle <app> off`, `netmeter resume-all`, `netmeter lowdata on|off`, `netmeter profile-here <name>`, `netmeter profile`, `netmeter profile rm <name>`, `netmeter networks`, `netmeter networks rm <name|mac>`, `netmeter networks on|off`, `netmeter recent 60`, `netmeter solo chrome`, `netmeter solo off`, `netmeter display --rate on|off --total on|off --combine on|off`, `netmeter show 2026-07-29`.
