...
# --- per-network totals, per day (2026-08-31) -------------------------------
# The tether counter answered "how much on the metered network". This is the
# same question asked of every network, on the same clock as Today, so that the
# three scopes in the menu are three spans you can hold against each other.
# Built per billing period first and changed: a menu reading Today 652 MB and
# Network 35 MB looks broken while being perfectly correct.

reset()
day = {"date": "2026-08-31", "started": None, "updated": None, "apps": {}}
nm.net_add(day, "aa:bb", 1000, 500, {"Chrome": [800, 400]}, name="Cafe")
nm.net_add(day, "aa:bb", 200, 100, {"Chrome": [150, 90], "Cursor": [40, 5]})
nm.net_add(day, "cc:dd", 7, 3, {"Chrome": [5, 2]})
e = day["nets"]["aa:bb"]
...
check("and a later tick without one does not clobber it", e.get("name") == "Cafe")
check("each network is counted on its own",
      day["nets"]["cc:dd"]["in"] == 7 and "Cursor" not in day["nets"]["cc:dd"]["apps"])
check("a tick with no MAC cannot be charged to a network",
      nm.net_add(day, "", 5, 5, {}) is None and len(day["nets"]) == 2)
check("the totals come back biggest first, so the caller need not re-sort",
      list(nm.net_totals(day)) == ["aa:bb", "cc:dd"])
check("and they are the interface numbers, not the app split",
      nm.net_totals(day)["aa:bb"] == 1800)
check("a network is stamped with when it first cost anything today",
      e.get("since", "").startswith(datetime.date.today().isoformat()))
first = e["since"]
nm.net_add(day, "aa:bb", 1, 1, {})
check("and the stamp is the first tick's, not the latest one's",
      day["nets"]["aa:bb"]["since"] == first)

# The day file is where they live, so they rotate with it and every past day
# keeps its own breakdown rather than one running figure that forgets.
nm.save_json(nm.state_path("2026-08-30"), day)
check("a day's networks survive a round trip through the day file",
      nm.load("2026-08-30")["nets"]["aa:bb"] == day["nets"]["aa:bb"])
check("and a day file written before any of this reads as no networks",
      nm.load("2019-01-01")["nets"] == {})
