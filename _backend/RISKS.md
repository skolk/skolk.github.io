...
- **Mitigation**: ROOT CAUSE FIXED 2026-06-17 (the surgical version of ADR-003's "relocate" option): Bundler was installing gems into `vendor/bundle/` *inside* the iCloud-synced repo, which is what kept getting evicted. Pointed `.bundle/config` (gitignored, laptop-local) at `BUNDLE_PATH: /Users/seankolk/.bundle/skolk-github-io`, ran `bundle install` there, and deleted the in-repo `vendor/`. The build now reads gems from outside iCloud and no longer has an evictable hot path; verified at ~1.4s, exit 0, 178 files (down from a 286s materialization-bound build minutes earlier). The repo itself stays in `~/Documents`. Defence in depth still in place: `./_backend/bin/build`'s 180s timeout and the pre-commit exit-124 handling remain as a backstop for any other evicted file (e.g. a cold image on first build). `brctl download "$(pwd)"` remains a manual fallback. Documented in CLAUDE.md (Gotchas).
- **Contract refs**: `.bundle/config` `BUNDLE_PATH` outside `~/Documents` (primary, removes the evictable gem tree) + `_backend/bin/build` 180s timeout + `.githooks/pre-commit` exit-124 handling (backstop).
- **Status**: Mitigated (root cause addressed; not archived because the fix is laptop-local config, a fresh clone defaults back to `vendor/bundle` until `.bundle/config` is set again, so the `bin/build` backstop still earns its keep). Was Open with the repo deliberately left in `~/Documents`; the bundle-path move resolved it without relocating the repo. Re-open if a build stalls again (would mean a non-gem file is being evicted).
