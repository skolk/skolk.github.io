...
### Changed
- Renamed the Salish orca map to the **Salish Migration Map** and moved it `salish-orcas/` → `salish/` (URL `/salish-orcas/` → `/salish/`). Left a redirect stub at `salish-orcas/index.html` (meta-refresh + JS, preserves query/hash). Updated the `/projects` link. The app is a self-contained static dir (relative asset paths), so the move is transparent; internal ids (`?site=salish-sea`, `salish-sea-landmask.json`) are unchanged.
...
### Added
- `/tarot/` gained a **day** theme and now carries three: day (warm paper, dark ink, for sunlight), dusk (the original cream-on-slate default), and night (red-light). The page was dark-only and unreadable outdoors, which is where it got opened first. The palette was already fully tokenized, so day is mostly one variable block; the two hardcoded dark panels (`.tape`, `.find` backgrounds at `rgba(0,0,0,.28)`) became a `--panel` token, and the filled draw button gets an outlined disabled state in day mode because cream-on-pale-pink at 30% opacity is invisible on paper. Theme classes moved from `body` to `<html>` so a small head script can resolve the theme before first paint, which kills the flash of dark. The button cycles and names the theme it will switch to; the choice persists in `localStorage`, and until one is made the page follows `prefers-color-scheme` and keeps following it if the system flips.
- `tarot/`, a self-contained tarot deck served at `/tarot/` like `/salish/` and `/facilitator/`, but a single HTML file with no build step and no vendored libraries: all 78 cards are drawn in inline SVG (22 hand-cut major emblems, pip layouts for the minors, plaque art for the courts), so nothing is fetched and it works offline. Shuffle is Fisher-Yates over the full deck sourced from `crypto.getRandomValues` with rejection sampling against modulo bias; reversals are decided per card at draw time; the cut takes a random depth and reports it. A tape across the top streams the consumed bytes with a running count. Spreads of one, two, or three places (Past/Present/Future or Situation/Action/Outcome), drawn one card at a time off a stack that actually depletes, plus a draw log, a grammar explainer (suit x number, and the four reversal conventions), and a searchable index of all 78. Night mode goes red-light rather than dim. Arrived as `_pages/projects/taro.html`, which would have published at `/projects/taro.html` and left the post's `/tarot/` link dead; moved to `tarot/index.html` and the `<title>` em-dash replaced with a middot. Write-up at `_posts/2026-08-24-Seventy-Eight-Cards.md` and a bullet on `/projects` under Active Projects. `taro/index.html` is a redirect stub at `/taro/` (the file's original spelling, and the short one to say out loud), meta-refresh plus JS, preserving query and hash, same pattern as `salish-orcas/`.
- `micmeter/`, a mic level meter for the macOS menu bar, sibling of `netmeter/` and built the same way (one swiftc-compiled bar app + `install.sh` + LaunchAgent). Two-row segment meter: raw mic level over the voice-processed level, so cafe noise that Voice Isolation is filtering shows as a lit top row over a dark bottom row. Menu reports noise floor, voice level, voice-over-background margin, the Control Center mic mode, and the measured dB the filter cuts off the background. Also absorbs the Mic Drop app's duties: a system-wide input mute on F5 (re-asserted every second, volume-zero fallback for devices without a mute switch, always restored on quit) and a default-input picker submenu. Menu is headed by a netmeter-style button row (Mute / Pause / dB / Bars); the meter is five segments; the voice-processing tap auto-suspends while any other process plays audio, because its echo canceller audibly degrades playback on a combined USB in/out device; and when default input and output are the same device, the whole meter yields during playback (bar shows `♪`), because opening a combined device's mic path flips its hardware DSP into speakerphone mode and drains the bass. Like `netmeter/`, not excluded from the Jekyll build, so the source is published with the site. Project page at `_pages/projects/micmeter.md` (`/projects/micmeter/`, image slot pending, tracked in IMAGE_GAPS) and a bullet on `/projects` under Active Projects.
- `_pages/drafts.html`, a `/drafts` listing page for the dev server. Lists everything in `_drafts/` (filtered from `site.posts` by `relative_path contains '_drafts'`) when serving with `--drafts`; production builds never include drafts, so the live page just shows an empty-state note. Not linked from nav.
- `_pages/contact.html`, a scrape-resistant `/contact` page. Contact values are XOR+base64 encoded and assembled in the browser only after a click on "Show", so no email or phone string exists in the published HTML source; a vCard download is built client-side as a Blob; an inline-SVG QR encodes `https://skolk.github.io/contact` (self-referential, for handing someone a phone screen). Currently holds the Island Lab address (never written in plaintext anywhere in this repo, this file included; decode the `ilab` value in the page's `var C` block if you need to read it back) plus plain links to islandlab.dev, LinkedIn, and Instagram; the personal-alias and phone rows wait on real values (the spec placeholders were deliberately not published). Nav gained a Contact entry; the unused `email:` key was dropped from `_config.yml` and the plaintext `mailto:` on `_pages/404.html` now points to `/contact`.
- Two hosted interactive sims, served as self-contained static dirs like `/salish/`: `/community-simulator/` (Community Economic Simulator) and `/facilitator/` (The Facilitator). Each is a single-file React sim precompiled with esbuild into a local `app.js` plus vendored React UMD (`react.js` / `react-dom.js`), so there is no build step and no runtime CDN. Source lives in `_backend/sim-sources/*.jsx` (not published); regenerate with `_backend/bin/build-sim <src.jsx> <Component> <out-dir> "<Title>"`.
- `_backend/bin/build-sim`: helper that compiles a single-file React sim (`import React` + `export default`) into a hosted dir. Strips the import/export, rebinds hooks off the global `React`, appends the mount, transpiles JSX with esbuild, and emits `index.html`.
- Three posts dated 2026-07-05 (all `reviewed_by_sean: false`): `Community-Economic-Simulator.md` and `The-Facilitator.md` (`type: project`, each links its live sim and carries a hero screenshot), and `Sharpen-a-Knife-on-a-Mug.md` (`type: update`, embeds the `knife-sharpening-mug-comic.svg` how-to). Added `/projects` bullets for both sims under Active Projects.
