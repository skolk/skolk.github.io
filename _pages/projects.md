...
- **[netmeter]({{ '/projects/netmeter/' | prepend: site.baseurl }}) (2026-)**: A macOS menu bar meter that answers who is spending the data when the boat is on a phone hotspot: live speed, per-app session and daily totals, and a switch beside each app to freeze it. The write-up covers the two `nettop` counter bugs that made the first version overcount by 4x.
