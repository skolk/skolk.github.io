...
- **[Salish Sea Orca Map]({{ '/salish-orcas/' | prepend: site.baseurl }}) (2026-)**: An interactive map of the lower Salish Sea forage web: where the salmon, herring, seals, and both orca ecotypes likely are, week by week. Deterministic seasonal models (calendar times place) with a weekly scrubber; the Southern Residents are derived from Chinook and Bigg's from harbor seal, so the map shows *why* the orcas are where they are. Groundwork for a whale-alert system that pings the phone when a sighting lands near the boat.
